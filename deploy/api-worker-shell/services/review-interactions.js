...
import { encodeFilterValue, getSupabaseKey, supabaseRequest } from '../lib/supabase.js';
...
async function uploadReviewFile(env, sessionUser, file) {
  ...
  const uploadUrl = `${env.APP_SUPABASE_URL}/storage/v1/object/${env.APP_SUPABASE_STORAGE_BUCKET}/${objectPath
    .split('/')
...
async function requireSessionUser(request, env, deps) {
...
export async function handleReviewUpload(request, env, deps) {
...
export async function handleCreateReview(request, env, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
  ...
  const stampRows = await supabaseRequest(
    env,
  ...
  const createdReview = await deps.loadSingleReview(env, insertedRows?.[0]?.feed_id, sessionResult.sessionUser.id);
  const createdNotification = await deps.createUserNotification(env, {
  ...
  if (createdNotification?.notification_id) {
    ...
    if (notification) {
      await deps.publishNotificationEvent(env, sessionResult.sessionUser.id, 'notification.created', {
        notification,
        unreadCount: await deps.countUnreadNotifications(env, sessionResult.sessionUser.id),
      });
    }
  }
...
export async function handleUpdateReview(request, env, reviewId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
...
export async function handleCreateComment(request, env, reviewId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
  ...
  if (parentComment && parentComment.user_id && parentComment.user_id !== actorUserId) {
    ...
    if (createdNotification?.notification_id) {
      const notification = await deps.loadNotificationById(env, createdNotification.notification_id);
      if (notification) {
        await deps.publishNotificationEvent(env, parentComment.user_id, 'notification.created', {
          notification,
          unreadCount: await deps.countUnreadNotifications(env, parentComment.user_id),
        });
      }
    }
  }
  ...
  if (reviewOwnerId && reviewOwnerId !== actorUserId && (!parentComment || parentComment.user_id !== reviewOwnerId)) {
    ...
    if (createdNotification?.notification_id) {
      const notification = await deps.loadNotificationById(env, createdNotification.notification_id);
      if (notification) {
        await deps.publishNotificationEvent(env, reviewOwnerId, 'notification.created', {
          notification,
          unreadCount: await deps.countUnreadNotifications(env, reviewOwnerId),
        });
      }
    }
  }
...
export async function handleUpdateComment(request, env, reviewId, commentId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
...
export async function handleDeleteComment(request, env, reviewId, commentId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
...
export async function handleDeleteReview(request, env, reviewId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
...
export async function handleToggleReviewLike(request, env, reviewId, deps) {
  const sessionResult = await requireSessionUser(request, env, deps);
  if (sessionResult.response) {
    return sessionResult.response;
  }
