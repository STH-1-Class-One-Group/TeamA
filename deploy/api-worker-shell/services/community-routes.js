...
import { buildInFilter, encodeFilterValue, supabaseRequest } from '../lib/supabase.js';
...
export function createCommunityRouteService({ loadStaticBaseRows }) {
  ...
  async function loadCommunityRoutes(env, options = {}) {
    ...
    const [{ placeRows }, userRows] = await Promise.all([
      ...
      userIdsFilter ? supabaseRequest(env, `user?select=user_id,nickname&user_id=${userIdsFilter}`) : Promise.resolve([]),
    ]);
  ...
  async function handleCreateUserRoute(request, env) {
    ...
    const sessionRows = await supabaseRequest(
      env,
      `travel_session?select=travel_session_id,user_id&travel_session_id=eq.${encodeFilterValue(travelSessionId)}&user_id=eq.${encodeFilterValue(sessionResult.sessionUser.id)}&limit=1`,
    );
    if (!sessionRows?.[0]) {
      return jsonResponse(404, { detail: '여행 세션을 찾지 못했어요.' }, env, request);
    }

    const existingRouteRows = await supabaseRequest(
      env,
      `user_route?select=route_id&user_id=eq.${encodeFilterValue(sessionResult.sessionUser.id)}&travel_session_id=eq.${encodeFilterValue(travelSessionId)}&limit=1`,
    );
    if (existingRouteRows?.[0]) {
      return jsonResponse(409, { detail: '이미 발행된 여행 코스예요.' }, env, request);
    }

    const sessionStampRows = await supabaseRequest(
      env,
      `user_stamp?select=position_id,created_at&travel_session_id=eq.${encodeFilterValue(travelSessionId)}&user_id=eq.${encodeFilterValue(sessionResult.sessionUser.id)}&order=created_at.asc`,
    );
    ...
    await supabaseRequest(env, 'user_route_place?select=user_route_place_id', {
      method: 'POST',
      body: JSON.stringify(
        orderedPositionIds.map((positionId, index) => ({
  ...
  return {
    ...
    handleMyRoutes,
    handleToggleCommunityRouteLike,
    loadCommunityRoutes,
  };
}
