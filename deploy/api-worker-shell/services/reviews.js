...
export function createReviewReadService({ formatVisitLabel, loadStaticBaseRows, mapPlace }) {
  ...
  function buildCommentTree(commentRows, usersById) {
    ...
    const collapseDeletedNodes = (nodes) =>
  ...
  async function loadReviewData(env, sessionUserId = null, filters = {}) {
    ...
    const reviewTravelSessionIds = [
      ...new Set((reviewStampRows ?? []).map((row) => row.travel_session_id).filter(Boolean).map((value) => String(value))),
    ];
  ...
  async function loadSingleReview(env, reviewId, sessionUserId = null) {
    ...
    const [commentRows, likeRows, placeRows, stampRows, userFeedLikeRows = []] = await Promise.all([
      supabaseRequest(
        env,
        `user_comment?select=comment_id,feed_id,user_id,parent_id,body,is_deleted,created_at&feed_id=eq.${encodeFilterValue(reviewId)}&order=created_at.asc`,
      ),
      ...
      reviewRow.stamp_id
        ? supabaseRequest(
            env,
            `user_stamp?select=stamp_id,user_id,position_id,travel_session_id,stamp_date,visit_ordinal,created_at&stamp_id=eq.${encodeFilterValue(reviewRow.stamp_id)}&limit=1`,
          )
        : Promise.resolve([]),
      sessionUserId
        ? supabaseRequest(
            env,
            `feed_like?select=feed_id&user_id=eq.${encodeFilterValue(sessionUserId)}&feed_id=eq.${encodeFilterValue(reviewId)}&limit=1`,
          )
        : Promise.resolve([]),
    ]);
    const reviewTravelSessionIds = [
      ...new Set((stampRows ?? []).map((row) => row.travel_session_id).filter(Boolean).map((value) => String(value))),
    ];
    const reviewRouteRows =
      reviewTravelSessionIds.length > 0
        ? await supabaseRequest(env, `user_route?select=route_id,travel_session_id&travel_session_id=${buildInFilter(reviewTravelSessionIds)}`)
