"""User-generated route repository based on travel sessions."""
...
def _load_route_or_raise(db: Session, route_id: str) -> UserRoute:
    try:
        ...
        raise RepositoryValidationError("경로 ID 형식이 올바르지 않아요.") from error
    ...
    if not route:
        raise RepositoryNotFoundError("경로를 찾지 못했어요.")
    return route
...
def create_user_route(db: Session, payload: UserRouteCreate, user_id: str, nickname: str) -> UserRouteOut:
    ...
    if len(title) < 2:
        raise RepositoryValidationError("경로 제목은 두 글자 이상으로 적어 주세요.")
    if len(description) < 8:
        raise RepositoryValidationError("경로 소개는 조금 더 자세히 적어 주세요.")

    try:
        travel_session_key = int(payload.travel_session_id)
    except ValueError as error:
        raise RepositoryValidationError("여행 세션 ID 형식이 올바르지 않아요.") from error
    ...
    if len(ordered_places) < MIN_ROUTE_PLACE_COUNT:
        raise RepositoryValidationError("코스에는 최소 두 곳 이상의 스탬프 기록이 필요해요.")
...
def toggle_user_route_like(db: Session, route_id: str, user_id: str, nickname: str) -> UserRouteLikeResponse:
    ...
    if not route.is_public:
        raise RepositoryValidationError("비공개 경로에는 좋아요를 누를 수 없어요.")
    if route.user_id == user_id:
        raise RepositoryValidationError("내가 만든 경로에는 좋아요를 누를 수 없어요.")
...
def delete_user_route(db: Session, route_id: str, user_id: str, *, is_admin: bool = False) -> None:
    ...
    if route.user_id != user_id and not is_admin:
        raise RepositoryPermissionError("내가 만든 경로만 삭제할 수 있어요.")
    db.delete(route)
    db.commit()
